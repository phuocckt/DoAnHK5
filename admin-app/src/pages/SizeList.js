  ...
  const handleSubmit = (e) => {
    ...
    axiosAdmin.post("/Sizes", sizeData)
      .then(() => {
        ...
        setIsAddSuccess(false);
        setShowAdd(false);
        setNotification('Thêm size thành công');
      })
      .catch((error) => {
        setNotification('Tên size trùng nhau');
      });
  };
  const handleDelete = (sizeId) => {
    axiosAdmin.delete(`/Sizes/${sizeId}`)
      .then(() => {
        // Sau khi xóa thành công, cập nhật danh sách sản phẩm
        setSize(prevSize => prevSize.filter(sizes => sizes.id !== sizeId));
        handleClose();
      });
  };
  ...
  const handleSaveEdit = (itemId) => {
    ...
    axiosAdmin
      ...
      .then(() => {
        ...
        setNotification('Cập nhật size thành công');
      })
      .catch((error) => {
      ...
      {showAdd && (
        <Form className="col-md-3" onSubmit={handleSubmit}>
          <Form.Group className="mb-3">
            ...
            <Form.Control type="text" name="name" onChange={handleChange} placeholder='Nhập tên size' required />
          </Form.Group>
          <Form.Check
            type="switch"
            name="status"
